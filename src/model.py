...
class regression_Transformer_basisModel(nn.Module):
    """
    Regression transformer class:
    - contains an input embedding layer, position embedding layer, transformer layers, and output layers
    - applies the transformer to a sequence of embeddings
    ...
    def __init__(
        ...
        super(regression_Transformer_basisModel, self).__init__()
        # give x.shape[1] as embedding_dim


        self.feed_forward = FeedForward(input_dim, dropout)
        self.linear_regression = Linear_regression(input_dim, output_dim)
